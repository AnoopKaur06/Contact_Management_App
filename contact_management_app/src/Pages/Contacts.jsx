...
const Contacts = () => {
    ...
    return (
        <div className="container mx-auto mt-16 px-4 text-gray-800">
            <div className="flex justify-between mb-4">
                <h1 className="text-3xl mt-4 font-bold">Contacts</h1>
                <Link to="/contact_form">
                    <button className="px-4 py-2 mt-4 bg-blue-600 text-white rounded-md ">
                        Create Contact
                    </button>
                </Link>
            </div>
            {AllContacts.length === 0 && (
                <div className="flex items-center justify-center p-8 bg-gray-100 rounded-md">
                    <svg className="w-16 h-16 mr-4 text-blue-500" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M10 3a7 7 0 00-7 7v3a1 1 0 001 1h12a1 1 0 001-1v-3a7 7 0 00-7-7zM5 10a5 5 0 1110 0 5 5 0 01-10 0z" clipRule="evenodd" />
                    </svg>
                    <p className="text-xl">No contacts found. Please add contacts using the button above.</p>
                </div>
            )}
                    ...
                    <div key={el.id} className="bg-white rounded-lg shadow-md">
                        <div className="p-4">
                            ...
                            <div className="text-center">
                                <p className="text-lg font-semibold">{el.first_name} {el.last_name}</p>
                                <p className="text-sm text-gray-500">{el.status === "active" ? "Active" : "Inactive"}</p>
                            </div>
                        </div>
                        <div className="flex justify-between p-4 border-t border-gray-200">
                            <Link to={`edit/${el.id}`}>
                                <button className="px-4 py-2 text-sm bg-blue-500 text-white rounded-md">Edit</button>
                            </Link>
                            <button onClick={() => dispatch(removeContact(el.id))} className="px-4 py-2 text-sm bg-red-500 text-white rounded-md">Delete</button>
                        </div>
                    </div>
                ))}
            </div>
